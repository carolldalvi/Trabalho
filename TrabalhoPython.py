...
while True:
    ...
    print("         --> Digite 3 para ALTERAR OS DADOS DO DEFUNTO")
    ...
    if option == 1:
        # Segunda lista, a lista cova, vai guardar informações sobre o defunto
        cova = []
        # Insere os dados via teclado
        quem_comprou = input("Nome de quem pagou o velório: ").upper()
        nome_defunto = input("Nome do defunto: ").upper()
        cor = input("Qual cor quer na lápide? ")
        frase = input("Qual frase quer na lápide? ")

        # adiciona as informações do defunto na lista cova, depois adiciona a lista cova, na lista cemitério
        cova.append(quem_comprou)
        cova.append(nome_defunto)
        cova.append(cor)
        cova.append(frase)
    ...
    # Se option for igual a 2 procura um lápide específica
        ...
        encontrou = False
        # A variavel pesquisa defunto, vai guardar o nome do defunto que vai pesquisar
        # Função upper deixa a letra maiuscula
        pesquisa_defunto = input(
            "Qual nome do defunto que quer procurar?").upper()
        # Percorre a lista cemitério
        for i in cemiterio:
            # Verifica se pesquisa_defunto é igual a um elemento da lista
            if pesquisa_defunto == i[1]:
                encontrou = True
                # Imprime as informações da cova
                print(
                    f'\nO defunto {i[1]} está descansando em paz em uma de nossas lápides, \nGOSTA DE SE JUNTAR A ELE? BRINCADEIRINHA!!!')
                print(f'Nome na lápide {i[1]}')
                print(f'Cor da lápide: {i[2]}')
                print(f'Frase na lápide: {i[3]}\n')
        # Se a variavel encontrou continua falsa é porque não encontrou nenhum defunto com aquele nome
        if(encontrou == False):
            print("Esse defunto não consta em nosso cemitério!")

    # Se option for igual a 3 altera algumas informações
    elif option == 3:
        encontrou = False
        pesquisa_defunto = input(
            "Qual nome do defunto que quer procurar para alterar?").upper()
        alteracao = int(
            input("O que seja alterar? 1 = FRASE NA LAPIDE, 2 = COR DA LÁPIDE : "))
        ...
        if(encontrou == False):
            print("Não foi possível alterar, este defunto não consta em nosso cemitério!")
    ...
    # Se option for igual a 4 imprime um relatório
        ...
        for i in cemiterio:
            ...
            # Na posicao 1 da lista dentro da lista cemiterio está o nome do defunto, e assim por diante
            # até a posição 3
            print(f'Nome do defunto: {i[1]}')
            print(f'Cor da lápide: {i[2]}')
            print(f'Frase na lápide: {i[3]}\n')
    ...
    # Se option for igual a 0 encerra o programa
        ...
        break
print('Trabalho feito por:Ana Carolina Lopes Dalvi, Gabriel Vianna Fracalossi, Nicolas Lima Rosário e Victor Perez Cabral')
